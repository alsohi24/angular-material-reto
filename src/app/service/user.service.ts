...
export class UserService {
  ...
  UserLogin(userdata: Usercred): Observable<UserSession> {
    return this.http.post<UserSession>(this.AuthUrl, {
  ...
  GetMenubyRole(userrole: string): Observable<Roleaccess[]> {
    return this.http.get<Roleaccess[]>(
      `${this.APIBaseUrl}roleaccess?role=` + userrole
    );
  }
  HaveMenuAccess(userrole: string, menuname: string): Observable<Roleaccess[]> {
    return this.http.get<Roleaccess[]>(
      `${this.APIBaseUrl}roleaccess?role=` + userrole + '&menu=' + menuname
    );
  }
  ...
  GetAllRoles(): Observable<Roles[]> {
    return this.http.get<Roles[]>('http://localhost:3001/role');
  }
